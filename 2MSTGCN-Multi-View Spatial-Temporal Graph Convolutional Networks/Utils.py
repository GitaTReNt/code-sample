...
def Instantiation_optim(name, lr, params):
    if name == "adam":
        ...
        raise ValueError('Config: check optimizer, may be not implemented.')
    return optimizer


def Instantiation_regularizer(l1, l2, params):
    if l1 != 0 and l2 != 0:
        # PyTorch does not support L1L2 regularizer directly like Keras,
        # You might need to implement custom regularization in your training loop
        regularizer = {'l1': l1, 'l2': l2}
    elif l1 != 0:
        regularizer = {'l1': l1, 'l2': 0}
    elif l2 != 0:
        regularizer = {'l1': 0, 'l2': l2}  # weight_decay in optimizer
    else:
        regularizer = None
    return regularizer

# Example of applying L2 regularization
# optimizer = Instantiation_optim('adam', 0.001, model.parameters())
# Example of using regularizer dict in a custom training loop
# for param in model.parameters():
#     loss += regularizer['l2'] * torch.sum(param ** 2) if regularizer['l2'] else 0
#     loss += regularizer['l1'] * torch.sum(torch.abs(param)) if regularizer['l1'] else 0
